...
async fn execute_set(args: &SetArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    labels.insert(
        "reinhardt.dev/provider".to_string(),
        args.provider.clone(),
    );
    ...
    println!("Secret '{secret_name}' applied in namespace '{}'", args.namespace);
    Ok(())
}
...
async fn execute_check(args: &CheckArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match secrets.get_opt(&secret_name).await? {
        ...
        None => {
            println!("Secret '{secret_name}' not found in namespace '{}'", args.namespace);
        }
    }
