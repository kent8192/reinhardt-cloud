...
mod tests {
	...
	fn diag_apply_migration_operations_from_state_no_spurious_altercolumn() {
		...
		// from either entry point. The upstream residual bug fixed by
		// reinhardt-web#4053 (closed reinhardt-cloud#476) is regression-
		// guarded here; the migration in 0005_add_constraint_clusters.rs is
		// now the verbatim autogenerated output.
		assert!(
