...
pub struct GitHubRepositoryImportRequest {
	#[validate(length(min = 1, message = "Select a repository"))]
	pub repository_id: String,
	#[validate(length(min = 1, message = "Select a cluster"))]
	pub cluster_id: String,
	#[validate(length(max = 63, message = "Project name must be 63 characters or fewer"))]
	pub project_name: String,
	#[validate(length(
		min = 1,
		max = 512,
		message = "Registry image prefix must be 1-512 characters"
	))]
	pub registry: String,
}
...
pub async fn import_github_repository_for_current_org(
	...
	use crate::apps::github::services::import::{
		...
		validate_project_name, validate_registry,
	};
	...
	let repository_id: i64 = repository_id
		.parse()
		.map_err(|_| ServerFnError::validation([("repository_id", "Select a valid repository")]))?;
	...
	let project_name = if project_name.trim().is_empty() {
		...
		validate_project_name(&project_name)
			.map_err(|message| ServerFnError::validation([("project_name", message)]))?
	};
	let registry = validate_registry(&registry)
		.map_err(|message| ServerFnError::validation([("registry", message)]))?;
	...
	let repository = GitHubRepository::objects()
		...
		.ok_or_else(|| {
			ServerFnError::validation([("repository_id", "Select a GitHub repository")])
		})?;
