...
use crate::apps::auth::client::routes::{
	LOGIN_PAGE_PATH, LOGIN_PAGE_ROUTE, REGISTER_PAGE_PATH, REGISTER_PAGE_ROUTE,
};
	...
	UnifiedRouter::new()
		.client(|c| {
			c.route("dashboard:home", "/", dashboard_shell)
				.route(LOGIN_PAGE_ROUTE, LOGIN_PAGE_PATH, login_page)
				.route(REGISTER_PAGE_ROUTE, REGISTER_PAGE_PATH, register_page)
