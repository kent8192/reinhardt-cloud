...
4. **No `AUTH_TOKEN` issuance on cluster registration.**
   `dashboard/src/apps/clusters/server_fn.rs` does not emit an Agent bearer
   token. `AUTH_TOKEN` must currently be minted manually using the JWT
   secret that `crates/reinhardt-cloud-grpc/src/interceptor.rs` validates
   against. Tracked in kent8192/reinhardt-cloud#361.
