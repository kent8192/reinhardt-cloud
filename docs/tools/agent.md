...
| `--auth-token` | `AUTH_TOKEN` | _(none)_ | No | Bearer token attached as `Authorization` header on every gRPC request; requires an HTTPS control-plane URL |
...
4. Deploy the agent pod referencing that secret and an `https://` control-plane URL.
   The agent rejects plaintext URLs when `AUTH_TOKEN` is configured so the
   bearer token is never sent over cleartext gRPC.
...
**Plaintext URL rejected when `AUTH_TOKEN` is set**

Authenticated agent connections must use `https://`. Either configure a TLS-terminating control-plane endpoint and update `CONTROL_PLANE_URL`, or unset `AUTH_TOKEN` for unauthenticated local-only development.
