...
`reinhardt-cloud` is the end-user command-line interface for the Reinhardt Cloud PaaS. It operates on one `Project` resource at a time. Standard deploys submit to the Dashboard control plane with an API token; direct cluster operations use the Kubernetes API.
...
Developer machine
  └── reinhardt-cloud CLI
        ├── (default) ──► Dashboard control plane ──► cluster agent ──► Project CRD
...
2. Environment variables (e.g. `REINHARDT_CLOUD_API_URL`)
...
| `--cluster <NAME>` | — | no* | — | Dashboard cluster name for standard deploys. Passed as `--context` to `kubectl` when `--direct` is used. |
...
\* `--name` and `--image` are required at runtime if they cannot be resolved from `manage introspect` output or `reinhardt-cloud.toml` (see resolution order below). `--cluster` is required for standard Dashboard deploys.
...
|------|---------|--------------------|-----------------|
| Standard Dashboard deploy | neither `--dry-run` nor `--direct` | Dashboard control plane via `/api/deployments/cli/`; the selected cluster agent applies the `Project` | Product deploy path with audit records and cluster selection |
| Direct (kubectl) | `--direct` | Kubernetes API server via `kubectl apply -f -` | Operator bootstrap or local cluster administration |
...
Standard mode requires an API token created by the Dashboard token-management flow. Run `reinhardt-cloud login --token <rct_...>` once, or set `REINHARDT_CLOUD_API_TOKEN` for one-off CI usage. In standard mode, `--cluster` is a Dashboard cluster name. In `--direct` mode, `--cluster` is a kubeconfig context.

For `--direct`, the CLI discovers the served CRD version from the target cluster unless `--api-version` is provided. For `--dry-run` and standard Dashboard deploys, it uses the explicit `--api-version` value when set, otherwise the compile-time default.
...
# 2. If validation passes, deploy through the Dashboard control plane
export REINHARDT_CLOUD_API_TOKEN="${REINHARDT_CLOUD_API_TOKEN:?set in CI secrets}"
reinhardt-cloud deploy \
  --name my-app \
  --image registry.example.com/my-app:${GIT_SHA} \
  --replicas 3 \
...
> **For Platform Operators**: Standard deploys require the target Dashboard cluster to have a connected agent and an installed `Project` CRD. `--direct` calls `kubectl apply` on the developer's machine, which means the developer needs `patch` / `create` RBAC on `projects` in the target namespace. When `--direct` is used with `--cluster`, the value is passed as `--context` to `kubectl`; ensure the developer's kubeconfig contains the named context. Pass `--api-version` only when discovery is unavailable or you intentionally need to pin a served CRD version.
...
- **`--cluster is required for dashboard deploys`** — Standard Dashboard deploys require a Dashboard cluster name. Pass `--cluster <NAME>`, or use `--direct` when you intend `--cluster` to mean a kubeconfig context.
- **`not logged in; run \`reinhardt-cloud login\` or set REINHARDT_CLOUD_API_TOKEN`** — Standard deploys need a bearer API token. Run `reinhardt-cloud login --token <rct_...>` or set `REINHARDT_CLOUD_API_TOKEN`.
- **`service unavailable: ...`** — The Dashboard accepted the request but could not enqueue the apply command for the selected cluster agent. Confirm the agent is connected and the cluster is active.
...
`status` currently uses the kubectl fallback path. The dashboard REST status operation is not exposed by the Pages app, so the command prints a warning and runs `kubectl get project <name> -n <namespace> -o json`, then renders the result locally.
...
Verify and persist a Dashboard API token for authenticated CLI commands.
...
|------|-------|------|----------|-------------|
| `--token` | `-t` | `string` | No | API token. Falls back to `REINHARDT_CLOUD_API_TOKEN`, then saved credentials, then an interactive prompt. |

Tokens are verified with `GET /api/auth/me/`. Successful login stores `credentials.json` under the platform config dir so later standard deploys can send bearer authentication automatically.

**Behavior**

1. Resolves an API token from the flag, `REINHARDT_CLOUD_API_TOKEN`, saved credentials, or stdin.
...
```bash
# Persist an API token for later deploy commands
reinhardt-cloud login --token rct_example
```

**CI pattern**

Prefer `REINHARDT_CLOUD_API_TOKEN` in CI so the token is scoped to the job environment and no credentials file needs to be written.

**Troubleshooting**

- **`token is invalid, expired, or revoked; generate a new one from the dashboard`** — Create a fresh API token in the Dashboard and retry.
- **`not logged in; run \`reinhardt-cloud login\` or set REINHARDT_CLOUD_API_TOKEN`** — No token was available from the flag, env var, saved credentials, or prompt.
...
### Dashboard authentication and status boundaries

**Symptoms**: standard `deploy` reports authentication or agent errors, while `status` reports an unsupported dashboard REST status operation and falls back to `kubectl`.

**Diagnosis**:

1. Check whether deploy has a bearer API token:
   ```bash
   reinhardt-cloud login --token <rct_...>
   ```
   Or set `REINHARDT_CLOUD_API_TOKEN` for the current shell or CI job.

2. Confirm standard deploy targets a Dashboard cluster name:
   ```bash
   reinhardt-cloud deploy --cluster production --name my-app --image ghcr.io/example/my-app:latest
   ```

3. Use `--direct` only when the CLI should bypass the Dashboard and apply the generated `Project` CRD with kubectl:
   ```bash
   reinhardt-cloud deploy --direct --name my-app --image ghcr.io/example/my-app:latest
   ```

4. Confirm cluster connectivity before direct mode:
   ```bash
...
- Use standard deploy with `--cluster <Dashboard cluster name>` for audited platform submission.
- Use `--direct` for local cluster administration or bootstrap.
- Use `status` with a working kubeconfig until Dashboard status REST is restored.
...
|----------|---------|---------|
| `REINHARDT_CLOUD_API_URL` | Dashboard control-plane target URL for authenticated CLI operations. | `http://localhost:8000` |
| `REINHARDT_CLOUD_API_TOKEN` | Bearer API token used by `login` and standard Dashboard deploys. | _(none)_ |
