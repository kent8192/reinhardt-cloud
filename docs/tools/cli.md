...
1. `reinhardt-cloud.toml` — always written (or overwritten with `--force`). Generated by `toml_generator::generate_config` + `generate_reinhardt_cloud_toml_string` based on the project name, version, Cargo features, optional database settings read from `settings/base.toml`, and runtime defaults for `[health]`, `[services]`, and `[scale]`.
...
- **Database section missing from `reinhardt-cloud.toml`** — `settings_reader::read_database_config` looks for `settings/base.toml` in `--dir`. If that file does not exist or does not contain a `[database]` table, no database section is emitted. Add the settings file or set the database config manually.
