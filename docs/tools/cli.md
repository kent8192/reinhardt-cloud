...
1. `reinhardt-cloud.toml` — always written (or overwritten with `--force`). Generated by `toml_generator::generate_config` + `generate_reinhardt_cloud_toml_string` based on the project name, version, Cargo features, and optional database settings resolved from `settings/base.toml` plus the active profile TOML.
