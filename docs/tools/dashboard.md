...
- **gRPC**: tonic 0.13 + tonic-reflection 0.13, hosting `AgentService` and `BuildService` on `127.0.0.1:50051` by default (`crates/reinhardt-cloud-grpc/src/config.rs`)
...
| gRPC server | tonic 0.13, default bind address `127.0.0.1:50051` | `crates/reinhardt-cloud-grpc/src/config.rs` |
