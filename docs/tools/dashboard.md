...
Managed application pods are written to Loki by the Helm chart's Promtail app scrape job when `logging.scrapeApps=true`. The Dashboard resolves the selected `deployment_id` through the current user's organization, enforces `LogsRead`, then maps the deployment's project name to the Loki `app` label and the deployment primary key to the Loki `deployment_id` label. Historical log requests use `deployment_logs_for_current_org`; live tailing uses the authenticated `/ws/notifications` WebSocket and sends `SubscribeAppLogs { deployment_id }`.

Filters available in the generic log service depend on `crates/reinhardt-cloud-proto/proto/log.proto` and `reinhardt_cloud_types::log::LogFilter`: `source` (project/app label), `min_level`, `since`, `until`, `search`, and `deployment_id`. The Dashboard managed-app log viewer sets both `source` and `deployment_id` after authorization so shared log backends constrain results to the selected deployment instead of relying on a user-controlled project name alone.
