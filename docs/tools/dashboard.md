...
Managed application pods are written to Loki by the Helm chart's Promtail app scrape job when `logging.scrapeApps=true`. The Dashboard resolves the selected `deployment_id` through the current user's organization, enforces `LogsRead`, then maps the deployment's project name to the Loki `app` label. The managed-app scrape path does not currently emit a Loki `deployment_id` label, so both historical reads and live tails query by the resolved project/source label. Historical log requests use `deployment_logs_for_current_org`; live tailing uses the authenticated `/ws/notifications` WebSocket and sends `SubscribeAppLogs { deployment_id }`.

Direct gRPC log reads must include a non-empty `source` or `deployment_id` filter so backend queries cannot default to a cross-application log selector. Filters available in the generic log service depend on `crates/reinhardt-cloud-proto/proto/log.proto` and `reinhardt_cloud_types::log::LogFilter`: `source` (project/app label), `min_level`, `since`, `until`, `search`, and `deployment_id`. The Dashboard managed-app log viewer sets `source`; `deployment_id` is used for authorization and lookup before the Loki query is built.
