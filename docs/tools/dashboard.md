...
Managed application pods must be written to Loki by tenant-scoped collectors that label streams with the project `app` and tenant `namespace`; the Helm chart's bundled Promtail collector is restricted to operator pods in the release namespace. The Dashboard resolves the selected `deployment_id` through the current user's organization, enforces `LogsRead`, then maps the deployment's project name to the Loki `app` label and the organization slug to the deterministic tenant namespace. Historical log requests use `deployment_logs_for_current_org`; live tailing uses the authenticated `/ws/notifications` WebSocket and sends `SubscribeAppLogs { deployment_id }`. Both paths include the tenant namespace in their gRPC log filter so project-name collisions in other organizations do not match the same Loki query.
