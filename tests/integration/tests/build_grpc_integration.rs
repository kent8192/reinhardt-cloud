...
async fn test_local_build_through_grpc_roundtrip() {
	...
	assert_eq!(
		phase_changes.len(),
		4,
		"Expected exactly 4 phase change events"
	);
	...
	assert_eq!(
		artifact_events.len(),
		1,
		"Expected exactly 1 artifact ready event"
	);
	...
	if let Some(pb::build_event::Event::ArtifactReady(ar)) = &artifact_events[0].event {
		assert!(
			!ar.artifact_url.is_empty(),
			"artifact_url should not be empty"
		);
		assert!(!ar.digest.is_empty(), "digest should not be empty");
		assert!(
			ar.digest.starts_with("sha256:"),
	...
	match &last.event {
		Some(pb::build_event::Event::Complete(c)) => {
			...
			assert!(
				c.timestamp.is_some(),
				"Complete event should have a timestamp"
			);
		}
